...
class WFInstaller extends JObject {
    ...
    function upgrade($version) {
        ...
        if (version_compare($version, '2.0.20', '<')) {
            ...
            $files  = JFolder::files($path, '\.js', false, true, array('jquery-1.7.1.min.js', 'jquery-ui-1.8.17.custom.min.js', 'jquery-ui-layout.js'));
            ...
            $query = 'SELECT id FROM #__wf_profiles WHERE rows LIKE %visualchars% AND plugins LIKE %visualchars%';
            $db->setQuery($query);
            
            $profile = JTable::getInstance('Profiles', 'WFTable');
            
            foreach ($db->loadResultArray() as $id) {
                $profile->load($id);
    ...
    function install($manual = false) {
        ...
        if ($state) {
            ...
            $message = '<div id="jce"><style type="text/css" scoped="scoped">' . file_get_contents(dirname(dirname(__FILE__)) . DS . 'media' . DS . 'css' . DS . 'install.css') . '</style>';
            
            $message .= '<h2>' . WFText::_('WF_ADMIN_TITLE') . ' ' . $version . '</h2>';
            $message .= '<ul class="install">';
            $message .= '<li class="success">' . WFText::_($installer->message, $installer->message) . '<li>';
            
            // install packages (editor plugin, quickicon etc)
            $packages   = $base . DS . 'packages';

            // install additional packages
            if (is_dir($packages)) {
                $message .= $this->installPackages($packages, true);
            }
    ...
    function _removeMenus() {
        ...
        $query = 'SELECT id FROM #__menu' . ' WHERE client_id = 1' . ' AND type = ' . $db->Quote('component') . ' AND path = ' . $db->Quote('jce') . ' AND component_id = 0';

        $db->setQuery($query);
        $id = $db->loadResult();

        if ($id) {
            $query = 'SELECT id FROM #__menu' . ' WHERE client_id = 1' . ' AND parent_id = ' . (int) $id;
    ...
    function removeTables($uninstall = false) {
        ...
        $db = JFactory::getDBO();
        $tables = array('plugins', 'extensions', 'groups', 'profiles');
    ...
    function cleanupDB() {
        ...
        $tables = array('plugins', 'profiles', 'groups', 'extensions');
     ...
     */
    function installPackages($source, $install = false) {
        jimport('joomla.installer.installer');
            ...
            if ($installer->install($source . DS . $folder)) {
                ...
                if ($install) {
                    if ($installer->message) {
                        $result .= '<li class="success">' . WFText::_($installer->message, $installer->message) . '</li>';
                    }
                } else {
                    $mainframe->enqueueMessage(WFText::_('WF_PACKAGE_INSTALL_SUCCESS'));
                }
                ...
                if (!$install) {
                    $mainframe->enqueueMessage(WFText::_('WF_PACKAGE_INSTALL_FAILED'));
                }
            }
        }
        ...
        foreach ($items as $type => $extensions) {
            if ($extensions) {
                foreach ($extensions as $id) {
                    ...
                    $mainframe->enqueueMessage($installer->message);
                }
            }
        }
    }
