...
class WFInstaller extends JObject {
	...
	function installPackages($source, $install = false) {
		...
		$packages = JFolder::folders($source, '.', false, true);
	...
	function removePackages() {
		...
		foreach($plugins as $folder => $elements) {
			foreach($elements as $element) {
				$items['plugin'][] = JPluginHelper::getPlugin($folder, $element);
			}	
		}

		foreach($modules as $module) {
			$items['module'][] = JModuleHelper::getModule($module);
		}
		...
		foreach($items as $type => $extensions) {
			if ($extensions) {
				foreach($extensions as $extension) {
					$installer = new JInstaller();	
					$installer->uninstall($type, $extension->id);
				}	
			}
		}
	}
